...
export abstract class CorePlugin {
   ...
   * */
  abstract match(attrs: AttributeMap, op: Op): boolean;
  /**
   * 渲染行节点
   * - 调度优先级值越大 DOM 结构在越外层
   */
  renderLine?(context: LineContext): P.Any;
  /**
   * 渲染块级子节点
   * - 调度优先级值越大 DOM 结构在越外层
   */
  renderLeaf?(context: LeafContext): P.Any;
  /**
   ...
   * 内容即将写入剪贴板
   */
  willSetToClipboard?(context: CopyContext): CopyContext;
  /**
   * 粘贴的内容即将应用到编辑器
   */
   ...
   */
  willPaintLineState?(lineState: LineState): void;
  /**
   * 编辑器行结构布局计算后异步调用
   */
  didPaintLineState?(lineState: LineState): void;
}
